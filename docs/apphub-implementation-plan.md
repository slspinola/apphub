# Plano de Implementação — apphub

## Visão Geral

O **apphub** é o hub central de um ecossistema de aplicações SaaS multi-tenant. Funciona como Identity Provider, gestor de organizações, utilizadores, permissões e licenças para todas as aplicações do ecossistema (bee2fleet, bee2vision, bee2energy, etc.).
...
**Objectivo:** Implementar sistema de autenticação completo para utilizadores do apphub.
...
model Permission {
  ...
  appId       String?  // null = permissão do apphub
...
**Objectivo:** Transformar o apphub num Identity Provider OAuth 2.0 / OpenID Connect para as apps do ecossistema.
...
### Entregáveis
- apphub como OAuth 2.0 Provider
...
**Objectivo:** Permitir registar e gerir as aplicações do ecossistema no apphub.
...
model App {
  ...
  isCore      Boolean   @default(false) // true para apphub
  metadata    Json      @default("{}")
  createdAt   DateTime  @default(now())
  updatedAt   DateTime  @updatedAt
...
## Fase 9 — SDK (@apphub/sdk)

**Objectivo:** Criar package npm que simplifica a integração das apps do ecossistema com o apphub.
...
- [ ] Criar AppHubProvider para NextAuth
...
- [ ] Métodos para chamar APIs do apphub
- [ ] Retry automático
- [ ] Gestão de erros
...
// middleware.ts
import { withAppHubAuth } from '@apphub/sdk/middleware'

export default withAppHubAuth({
...
import { AppHubProvider } from '@apphub/sdk/auth'
...
import { getTenantContext, requirePermission } from '@apphub/sdk'
...
import { PermissionGate } from '@apphub/sdk/components'
...
**Objectivo:** Criar interface completa para administração do apphub.
...
AppHub funcional para gestão interna de organizações, utilizadores e permissões. Ainda sem integração com apps externas.
...
AppHub pronto para integrar apps do ecossistema. SDK publicado.
